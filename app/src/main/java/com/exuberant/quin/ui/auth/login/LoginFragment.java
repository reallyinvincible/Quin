...
public class LoginFragment extends Fragment {
    ...
    private void addAccount(User user){
        ...
        Account account = new Account(user.getName(), getString(R.string.account_type));
        boolean success = accountManager.addAccountExplicitly(account, user.getHashedPassword(),null);
        if(success){
            AuthActivity.getFragmentControlInterface().showSnackbar("Account created", 1);
        }else{
            AuthActivity.getFragmentControlInterface().showSnackbar("Account creation failed", 0);
        }
    }
